...
void CandidatesGenerator::preprocessPointCloud(util::Cloud& cloud, const Eigen::Isometry3d& transform_base_opt) {
  printf("Processing cloud with %zu points.\n",
         cloud.getCloudOriginal()->size());

  cloud.removeNans();

  cloud.filterWorkspace(params_.workspace_, transform_base_opt);
